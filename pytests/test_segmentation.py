...
def test_speaker_diarization():
    labels, purity_cluster_m, purity_speaker_m = \
        aS.speaker_diarization("G:\pyAudioAnalysis\pyAudioAnalysis\data\diarizationExample.wav", 
                                4, plot_res=False)
...
def test_mt_file_classification():
    labels, class_names, accuracy, cm = aS.mid_term_file_classification(
                                     "G:\pyAudioAnalysis\pyAudioAnalysis\data\scottish.wav", 
                                     "G:\pyAudioAnalysis\pyAudioAnalysis\data\models\svm_rbf_sm", "svm_rbf", False, 
                                     "G:\pyAudioAnalysis\pyAudioAnalysis\data\scottish.segments")
    assert accuracy > 0.95, "Segment-level classification accuracy is low"

if __name__ == "__main__":    
    test_speaker_diarization()
    test_mt_file_classification()
