...
def segments_to_labels(start_times, end_times, labels, window):
    """
    Convert segment endpoints and respective segment labels to fix-sized class labels.
    Arguments:
...
def read_segmentation_gt(gt_file):
    """
    Read the ground truth segmentation from a CSV file.
    Arguments:
    gt_file -- Path to the ground truth file (CSV)
    Returns:
    seg_start -- List of segment start times
    seg_end -- List of segment end times
    seg_labs -- List of segment labels
    """
    try:
        import csv
        seg_start = []
...
def train_hmm_from_file(wav_file, gt_file, hmm_model_name, mid_window, mid_step):
    ...
    print(f"Segment start times: {seg_start}")
    print(f"Segment end times: {seg_end}")
    print(f"Segment labels: {seg_labs}")

    print("Extracting features from audio file...")
    sampling_rate, signal = audioBasicIO.read_audio_file(wav_file)
    features, _, _ = mtf.mid_feature_extraction(
    ...
    print(f"Saving HMM model to {hmm_model_name}")
...
def train_hmm_from_directory(folder_path, hmm_model_name, mid_window, mid_step):
    ...
    a directory where WAV files and .segment (ground-truth files) are stored.
    """
    flags_all = np.array([])
    class_names_all = []
...
def save_hmm(hmm_model_name, hmm, class_names, mid_window, mid_step):
    ...
    try:
        ...
        print(f"Error: Permission denided while tryin to write the file named {hmm_model_name}.", e)
    except pickle.PicklingError as e:
        print(f"Error: Failed to pickle object.", e)
    except Exception as e:
        print("An unexpected error while save the model.", e)
        
# For ex purpose.
# save_hmm_model('path/to/hmm_model.pkl', hmm, class_names, mid_window, mid_step)
