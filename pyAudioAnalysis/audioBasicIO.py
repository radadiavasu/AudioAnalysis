...
def read_aif(path):
    ...
    try:
        with aifc.open(path, 'r') as s:
            #--------------------------------------------->
            # Please visit `aifc.py` file for more deatils.
            # upto 64 to 82 lines.
            #<--------------------------------------------#
            nframes = s.getnframes() # Returns number of audio frames.
            strsig = s.readframes(nframes) # Returns most n frames of audio.
